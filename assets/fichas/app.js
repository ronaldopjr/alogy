(function () {
  ...
  function capture(){
    if(kind==='balanca'&&$('test').value!=='cargas'){$('mode').value='count';$('repeats').value='1';$('direction').value='Sequência informada';}
    return {version:1,kind,id:uid(),unit:$('unit').value.trim(),settings:{low:'',high:'',mode:kind==='ph'?'manual':'count',count:'5',step:'10',manual:'',repeats:'3',digits:'2',...Object.fromEntries(settingsFields.map(f=>[f.dataset.setting,f.value]))},meta:Object.fromEntries(metaFields.filter(f=>f.id!=='unit').map(f=>[f.dataset.meta,f.value])),readings:{}};
  }
  const initial=capture(); let state=structuredClone(initial), points=[], pages=[], timer, valid=false, renderedKey=null, lastSaved=null, pdfBusy=false;
  const say=message=>{$('save-status').textContent=message;};
  function save(){
    try{
      if(localStorage.getItem(key)!==lastSaved){say('O rascunho foi alterado em outra aba. Baixe um backup desta ficha antes de recarregar; os dados da outra aba foram preservados.');return false;}
      const serialized=JSON.stringify(state);localStorage.setItem(key,serialized);lastSaved=serialized;
      say('Rascunho salvo neste aparelho. Baixe um backup para conservar uma cópia.');return true;
    }catch{say('Este navegador não conseguiu salvar o rascunho. Baixe um backup antes de sair.');return false;}
  }
  ...
  function sanitize(raw){
    ...
    function date(v){if(!v)return;const d=new Date(v+'T00:00:00Z');if(!/^\d{4}-\d{2}-\d{2}$/.test(v)||!Number.isFinite(d.getTime())||d.toISOString().slice(0,10)!==v)throw Error('Data inválida no backup.');}
    next.id=typeof raw.id==='string'&&raw.id.length<=100?raw.id:uid();next.unit=string(raw.unit,20);
    if(kind==='ph'&&next.unit!=='pH')throw Error('A unidade do backup de pH deve ser pH.');
    for(const k of Object.keys(next.settings))next.settings[k]=string(raw.settings[k],k==='manual'?1500:120);
    for(const k of Object.keys(next.meta))next.meta[k]=string(raw.meta[k]??'',k==='notes'?1200:120);
    date(next.meta.date);
    for(const k of ['timeStart','timeEnd'])if(next.meta[k]&&!/^(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d)?$/.test(next.meta[k]))throw Error('Horário inválido no backup.');
    ...
    const allowedDirections=['Sequência informada','Subida','Descida','Subida e descida'];
    ...
    if(kind==='balanca'&&(!['cargas','repetibilidade','excentricidade'].includes(next.settings.test)||(next.settings.test!=='cargas'&&(next.settings.repeats!=='1'||next.meta.direction!=='Sequência informada'||next.settings.mode!=='count'))))throw Error('Configuração de ensaio da balança inválida no backup.');
    if(Array.isArray(raw.readings)||Object.keys(raw.readings).length>100)throw Error('Backup excede o limite de leituras ou tem formato inválido.');
    const keys=Object.keys(raw.readings).length?new Set(C.sequence(C.points(next.settings,kind),next.meta.direction).map(r=>r.index+':'+r.direction)):new Set();
    for(const [k,r] of Object.entries(raw.readings)){
      if(!keys.has(k)||!r||!Array.isArray(r.pairs)||r.pairs.length>Number(next.settings.repeats)||r.pairs.some(p=>!p||typeof p!=='object'||Array.isArray(p)))throw Error('Leituras incompatíveis com os pontos, sentidos ou repetições do backup.');
      ...
      date(next.readings[k].validity);
    }
    return next;
  }
  function sequence(){
    return C.sequence(points,state.meta.direction);
  }
  ...
  function renderCards(){
    $('reading-cards').innerHTML=sequence().map(({point,index,direction})=>{
      ...
      return `<section class="reading-card" data-row-card="${k}"><div class="reading-title">${point.label||'Ponto '+(index+1)} · ${C.format(point.nominal,Number(state.settings.digits))} ${C.escape(state.unit)} · ${direction}</div>${buffer}${pairs}<div class="reading-summary" data-summary="${k}"></div><label class="field"><span>${kind==='vazao'?'Observações / volume ou massa / tempo de coleta':kind==='balanca'?'Pesos utilizados / posição na plataforma / observações':'Observações do ponto'}</span><textarea data-row="${k}" data-property="note" maxlength="160" rows="2">${C.escape(row.note||'')}</textarea></label></section>`;
    }).join('');
    updateStats();
  }
  ...
  function preparePoints(){
    ...
    if(!(kind==='balanca'&&state.settings.test!=='cargas')&&new Set(labels).size!==labels.length)throw Error('Há pontos que ficam iguais com este arredondamento. Aumente as casas decimais ou ajuste a lista de pontos.');
  }
  function check(){
    ...
    if(kind==='balanca')for(const k of ['zeroStart','zeroEnd'])if(state.meta[k]?.trim()&&C.number(state.meta[k])===null)throw Error('Confira a indicação de zero da balança ou deixe o campo em branco.');
  ...
  function setMode(){
    ...
    if(kind==='balanca'){
      const fixed=state.settings.test!=='cargas';
      $('testLoad').closest('label').hidden=!fixed;
      $('mode').closest('label').hidden=fixed;
      $('edit-points').hidden=fixed;
      $('repeats').disabled=fixed;$('direction').disabled=fixed;
    }
  }
  function render(rebuild=true){
    setMode();let prepared=false;
    try{
      preparePoints();prepared=true;
      const currentKey=C.fingerprint(state)+':'+state.settings.digits;
      if(rebuild||renderedKey!==currentKey){renderCards();renderedKey=currentKey;}
      check();valid=true;$('form-error').hidden=true;
      $('point-summary').innerHTML=points.map((p,i)=>`<div class="point-chip"><span>${p.label||(p.percent===null?'Ponto '+(i+1):C.format(p.percent,2)+'%')}</span><strong>${C.format(p.nominal,Number(state.settings.digits))} ${C.escape(state.unit)}</strong></div>`).join('');
      ...
      $('page-info').textContent=`${points.length} ${kind==='balanca'&&state.settings.test!=='cargas'?'ciclos / posições':'pontos'} · ${sequence().length} linhas de coleta · ${pages.length} folha(s) A4 em paisagem`;
    }catch(error){valid=false;if(!prepared){$('reading-cards').innerHTML='';renderedKey=null;}pages=[];$('form-error').hidden=false;$('form-error').textContent=error.message;$('point-summary').innerHTML='';$('preview').innerHTML='';$('print-root').innerHTML='';$('page-info').textContent=error.message;}
    for(const id of ['download-pdf','print-sheet','preview-toggle'])$(id).disabled=!valid;
    if(pdfBusy)$('download-pdf').disabled=true;
  }
  function readForm(){
  ...
  $('download-pdf').addEventListener('click',async()=>{
    if(pdfBusy)return;clearTimeout(timer);render(false);if(!valid)return;
    const button=$('download-pdf'),snapshot=pages,name=filename('pdf');button.disabled=true;button.textContent='Preparando PDF…';
    pdfBusy=true;try{download(await P.pdf(snapshot),'application/pdf',name);}catch(e){say(e.message);}finally{pdfBusy=false;button.textContent='Baixar PDF';button.disabled=!valid;}
  });
  ...
  $('backup-file').addEventListener('change',async event=>{
    ...
    try{if(file.size>1024*1024)throw Error('Backup muito grande. O limite é 1 MB.');const next=sanitize(JSON.parse(await file.text()));
      ...
      clearTimeout(timer);state=next;populate();render();if(save())say('Backup aberto e salvo neste aparelho. Confira a ficha antes de imprimir.');
  ...
  $('duplicate').addEventListener('click',()=>{
    ...
    state.readings={};state.id=uid();for(const k of ['tag','record','serial','date','operator','notes','condition','channel','tempStart','tempEnd','humidityStart','humidityEnd','timeStart','timeEnd',...(kind==='balanca'?['tare','zeroStart','zeroEnd']:[])])state.meta[k]='';
    populate();render();if(save())say('Nova ficha salva: configuração mantida; identificação específica, condições ambientais e leituras limpas.');
  });
  $('clear').addEventListener('click',()=>{if(!window.confirm('Limpar a ficha deste aparelho? Baixe um backup se precisar conservar os dados.'))return;state=structuredClone(initial);state.id=uid();populate();render();try{if(localStorage.getItem(key)!==lastSaved){say('Ficha limpa nesta tela. O rascunho alterado em outra aba foi preservado.');return;}localStorage.removeItem(key);lastSaved=null;say('Ficha limpa. Os dados não foram enviados a um servidor.');}catch{say('Ficha limpa nesta tela. O navegador não permitiu alterar o armazenamento.');}});
  try{const saved=localStorage.getItem(key);lastSaved=saved;if(saved){state=sanitize(JSON.parse(saved));populate();say('Rascunho anterior recuperado neste aparelho. Baixe um backup para guardá-lo.');}}
